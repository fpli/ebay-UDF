...
public class TrieExt<T> implements StringSearcherExt<T>, StringSearcherPrepareExt<T> {

    private final StringSearcherConfig trieConfig;

    private final State<T> rootState;

    public TrieExt(final StringSearcherConfig trieConfig) {
     ...
     *
     * @param text The text to search for keywords.
     * @param startPos The starting point of text to match
     * @return null if no matches found.
     */
    public Emit<T> firstMatch(final CharSequence text, int startPos) {
        if (!trieConfig.isAllowOverlaps()) {
            ...
            for (int position = startPos; position < text.length(); position++) {
                Character character = text.charAt(position);
