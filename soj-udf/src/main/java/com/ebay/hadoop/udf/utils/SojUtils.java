...
public class SojUtils {
    ...
    private static Map<String, String> decodeAndParse(String payload, Map<String, List<String>> searchs, int decodeLevel) {

        Map<String, String> payloadMap = new LinkedHashMap<String, String>();

        String firstDecodedUrl;
        try {
            if(decodeLevel == 0 ) firstDecodedUrl = payload;
            else firstDecodedUrl = java.net.URLDecoder.decode(payload, "UTF-8");
        } catch (Exception e) {
        ...
        for (String key : searchs.keySet()) {
            ...
            if (isEmpty(payloadMap.get(key))) {
                if (decodeLevel == 2) {
                    try {firstDecodedUrl = java.net.URLDecoder.decode(firstDecodedUrl, "UTF-8"); } catch (UnsupportedEncodingException e) { }
