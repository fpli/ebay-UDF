 ...
 */ 
class SrpGistIsNull extends UDF {
  // lazy val srpDecoder =
  //   new BinaryMessageDecoder[Srp](new IsNullificData(), Srp.SCHEMA$)
  lazy val srpDecoder = Decoders.srpGist

  /**
   * This method checks if a SRP is NULL.
   *
   * @return 1 if the input SRP is NULL, 0 for all other situations including exception
   */
